...
RSpec.describe User, type: :model do
  ...
  describe 'relationships' do
    it 'has many secrets' do
      user = create_user
      secret1 = user.seeks.create(content: 'secret 1')
      secret2 = user.seeks.create(content: 'secret 2')
      expect(user.seeks).to include(secret1)
      expect(user.seeks).to include(secret2)
    end
    it 'has many likes' do
      user = create_user
      secret1 = user.seeks.create(content: 'Oops')
      secret2 = user.seeks.create(content: 'I did it again')
      like1 = Like.create(user: user, seek: secret1)
      like2 = Like.create(user: user, seek: secret2)
